...
export function isNegative(bignum: Uint8Array | undefined) {
  return (
    !!bignum && bignum.length > 0 && (bignum[bignum.length - 1]! & 0x80) !== 0
  );
}
 ...
 * @param minDigits 0-pad result to this many digits
 */
export function binaryToBase58(bignum: Uint8Array) {
...
function stringToKey(
  ...
  const dataToBuffer = Buffer.isBuffer(dataBuffer)
    ? dataBuffer
    : Buffer.from(dataBuffer);
  const digest = new Uint8Array(dataToBuffer);
  if (
    digest[0] !== whole[size + 0] ||
...
function keyToString(key: Key, suffix: string, prefix: string) {
  ...
  const keyDataToBuffer = Buffer.isBuffer(keyDataBuffer)
    ? keyDataBuffer
    : Buffer.from(keyDataBuffer);
