...
const checkEncrypt = ({
  ...
  const Ke =
    K instanceof Buffer ? K.subarray(0, 32) : Buffer.from(K).subarray(0, 32); // Encryption
...
const getSharedSecret = ({
  ...
  const encryptionPublicKeyBuffer = checkDecode({
    keyString: encryptionPublicKey,
  });
...
export const getCipherContent = ({
  ...
  const cipherbuffer = checkEncrypt({
    secret:
      sharedSecret instanceof Buffer ? sharedSecret : Buffer.from(sharedSecret),
    message,
  });
...
export const getUncipherContent = ({
  ...
  const message = checkDecrypt({
    secret: Buffer.isBuffer(sharedSecret)
      ? sharedSecret
      : Buffer.from(sharedSecret),
    message: Buffer.from(content, 'base64'),
  });
